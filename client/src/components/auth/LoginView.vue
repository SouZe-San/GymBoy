...
:class="{ 'active': userType === 'author' }"
        ...
        <h5>
        New Here?
          <span > <RouterLink to="/auth?type=register" style="text-decoration: none; color: inherit;">Sign In</RouterLink></span>
        </h5>
      </div>    
      <span class="shadow"></span>
        </form>
...
//   if (btnRef.value) {
//     const movable = btnRef.value;
// const parent = document.querySelector('.auth-form');
//     btnRef.value.addEventListener('mousemove', (event) => {
...
//     const distanceX = mouseX - objX;
//     const distanceY = mouseY - objY;
//     const distance = Math.sqrt(distanceX * distanceX + distanceY * distanceY);

//     // Set a threshold distance to move the object
//     const threshold = 100; // pixels

//     if (distance < threshold) {
//         // Calculate new position for the movable object
//         const angle = Math.atan2(distanceY, distanceX);
//         const newX = objX - Math.cos(angle) * (threshold - distance);
//         const newY = objY - Math.sin(angle) * (threshold - distance);

//         // Move the object to the new position
//         movable.style.left = `${newX - rect.width / 2}px`;
//         movable.style.top = `${newY - rect.height / 2}px`;
//     }
// });
</script>
